...
async function handleContentMessage(instance: CastInstance, message: Message) {
  ...
  switch (message.subject) {
    case "main:initializeCastSdk": {
      ...
      if (instance.apiConfig.autoJoinPolicy === AutoJoinPolicy.PAGE_SCOPED) {
        break;
      }
    ...
    case "main:requestSessionById": {
      ...
      if (!session) {
        logger.log(`Session not found! (id: ${message.data.sessionId})`);
        break;
      }
    ...
    case "main:leaveSession": {
      ...
      if (target) {
        ...
        for (const activeInstance of activeInstances) {
          if (
            (activeInstance === instance || activeInstance.session?.appId) !==
            sessionAppId
          )
            continue;

          if (isValidAutoJoinContext(activeInstance, target.autoJoinContext)) {
...
function createSelector() {
  ...
  ) => deviceManager.sendReceiverMessage(ev.detail.deviceId, ev.detail.message);
...
async function createMirroringPopup(device: ReceiverDevice) {
  ...
  const onMirroringPopupMessage = (port: Port) => {
    if (port.sender?.tab?.windowId !== popup.id || port.name !== "mirroring") {
      return;
    }

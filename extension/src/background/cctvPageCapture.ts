...
export function beginCctvPageCapture(
  ...
  if (!state.retryTimer) {
    state.retryTimer = setInterval(() => {
      if (state.armed && state.pending.length > 0) void flushPending(state);
    }, 2000);
  }
...
export function pauseCctvPageCaptureIngest(
  tabId: number,
  requestId: string
) {
  const state = tsCaptureByTab.get(tabId);
...
export async function armCctvPageCaptureInTab(tabId: number) {
  try {
    if (typeof (browser.webRequest as any).filterResponseData !== "function") {

...
function parseMasterVariants(raw: string, masterUrl: string): MasterVariant[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const height = Number(attrs.match(/(?:^|,)RESOLUTION=\d+x(\d+)/i)?.[1]) || undefined;
    variants.push({ url: new URL(uri.trim(), masterUrl).href, bandwidth, height });
  }
  return variants;
}
...
function pickMasterVariant(
  ...
  const candidates = requestedHeight
    ? variants.filter((variant) => variant.height !== undefined && variant.height <= requestedHeight)
    : variants;
...
function selectMasterVariant(
  ...
  const best = pickMasterVariant(parseMasterVariants(raw, masterUrl), requestedHeight);
...
function publishCapturedMaster(tabId: number, captured: CapturedMasterPlaylist) {
...
function rememberCdrmldSeed(tabId: number, url: string) {
  try {
    ...
    const dirMatch = parsed.pathname.match(/^(.*\/)cdrmld[a-z0-9]+_1\/index\.m3u8$/i);
    if (dirMatch) {
...
function captureLivePlaylist(tabId: number): Promise<CapturedMasterPlaylist> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error("No cached master playlist with RESOLUTION for this CCTV page"));
    }, CAPTURE_TIMEOUT_MS);
  ...
  XMLHttpRequest.prototype.open = function (
    ...
    if (/\.m3u8(?:[?#]|$)/i.test(absoluteUrl)) {
      this.addEventListener("load", () => {
        ...
        const kind = /^#EXT-X-STREAM-INF:.*(?:^|,)RESOLUTION=\d+x\d+(?:,|$)/im.test(raw)
          ? "master"
...
function initMasterPlaylistResponseCapture() {
  browser.webRequest.onBeforeRequest.addListener(
    (details) => {
      ...
      const filterResponseData = (browser.webRequest as any).filterResponseData as
        | ((requestId: string) => {
      ...
      try {
        ...
        logger.warn("CCTV response filter unavailable; using XHR fallback", {
          error: error instanceof Error ? error.message : String(error),
        });
        return;
      }
      ...
      filter.onstop = () => {
        ...
        if (isMasterPlaylist(raw)) {
          publishCapturedMaster(details.tabId, { url: details.url, raw });
          logger.info("CCTV master playlist captured from page response", {
            tabId: details.tabId,
            url: details.url,
          });
          ...
          const isNewUrl = lastLoggedMediaUrlByTab.get(details.tabId) !== details.url;
          publishCapturedMedia(details.tabId, details.url);
          if (isNewUrl) {
            lastLoggedMediaUrlByTab.set(details.tabId, details.url);
            logger.info("CCTV media playlist captured from page response", {
              tabId: details.tabId,
              url: details.url,
            });
          }
    ...
    { urls: ["*://*/*.m3u8*"], types: ["xmlhttprequest", "media", "other"] },
    ["blocking"]
  );
...
async function resolveLiveStreamUrl(tabId: number): Promise<string> {
  ...
  if (media && !isEnc1Delivery(media.url)) {
    logger.info("CCTV live stream resolved from the page's own media playlist", {
      tabId,
      mediaUrl: media.url,
    });
    return media.url;
  ...
  if (streamId) {
    ...
    for (const base of baseCandidates) {
      ...
      try {
        ...
        if (!response.ok) {
          probes.push({ base, ok: false, detail: `HTTP ${response.status}` });
          continue;
        }
        const raw = await response.text();
        if (!/^#EXT-X-STREAM-INF/im.test(raw)) {
          probes.push({ base, ok: false, detail: "not a master playlist" });
          continue;
        }
    ...
    logger.info("CCTV live stream resolved via the cdrmld (H.264) delivery", {
      tabId,
      streamId,
...
const CDRMLD_BASE_BY_CDN_FAMILY: Array<{ match: RegExp; base: string }> = [
  { match: /\.bdydns\.com$/i, base: "https://ldcctvwbcdbd.a.bdydns.com/ldcctvwbcd" },
  { match: /\.volcfcdn\.com$/i, base: "https://ldcctvwbcdbyte.volcfcdn.com/ldcctvwbcd" },
  { match: /\.live\.cntv\.cn$/i, base: "https://ldcctvwbcdtxy.liveplay.myqcloud.com/ldcctvwbcd" },
  { match: /\.liveplay\.myqcloud\.com$/i, base: "https://ldcctvwbcdtxy.liveplay.myqcloud.com/ldcctvwbcd" },
  { match: /\.kcdnvip\.com$/i, base: "https://ldcctvwbcdks.v.kcdnvip.com/ldocctvwbcd" },
  { match: /\.wscdns\.com$/i, base: "https://ldcctvwbcdcnc.v.wscdns.com/ldncctvwbcd" },
];

function cdrmldBaseForPageHost(host: string): string | undefined {
  return CDRMLD_BASE_BY_CDN_FAMILY.find((entry) => entry.match.test(host))?.base;
}
...
function liveStreamIdFromUrl(url: string): string | undefined {
  try {
    return new URL(url).pathname.match(/^\/live\/([a-z0-9_]+)\/?$/i)?.[1]?.toLowerCase();
  } catch {
    return undefined;
  }
}
...
function mediaMatchesVariant(mediaUrl: string, variantUrl: string): boolean {
  try {
    ...
    if (mediaBR !== null || variantBR !== null) return mediaBR === variantBR;
...
async function ensurePageVariant(tabId: number, requestedHeight: number) {
  ...
  if (!(await clickPageResolutionItem(tabId, label))) {
    logger.warn("CCTV resolution item not found on the page", { tabId, label });
  ...
  browser.webRequest.onBeforeRequest.addListener(
    (details) => {
      ...
      filter.onstop = () => {
        filter.disconnect();
        if (heartbeatOnly) {
          handleCapturedCctvTsSegment(details.tabId, details.url, new ArrayBuffer(0));
          return;
        }
  ...
  browser.runtime.onMessage.addListener((message, sender) => {
      ...
      if (
        ...
        if (kind === "media") {
          if (/^#EXTM3U/im.test(raw) && /^#EXTINF/im.test(raw)) {
            ...
            logger.info("CCTV media playlist captured by XHR fallback", {
              tabId,
              url,
            });
          }
          ...
          logger.info("CCTV master playlist captured by XHR fallback", {
            tabId,
            url,
          });
        }
    ...
    if (message?.subject === "cctv:capturedTsSegment") {
      ...
      if (
        ...
        try {
          ...
          logger.warn("CCTV captured TS segment failed to decode", { tabId });
        }
      }
      return;
    }
    if (message?.subject === "cctv:getQualityChoices") {
      ...
      return (async () => {
        ...
        const master =
          tab?.id !== undefined ? capturedMasterByTab.get(tab.id) : undefined;
        if (!master) return { choices: [] };
        const byHeight = new Map<number, MasterVariant>();
        for (const variant of parseMasterVariants(master.raw, master.url)) {
        ...
        const choices = [...byHeight.values()]
          ...
          .flatMap((variant) =>
            variant.height === undefined
              ? []
              : [{ height: variant.height, label: pageQualityLabel(variant.height) }]
          );
        logger.info("CCTV quality choices served from captured master", {
          tabId: tab?.id,
          choices,
        });
        return { choices };
    ...
    return (async () => {
      ...
      try {
        ...
        logger.error("CCTV live stream resolution failed", { tabId, reason });
        ...
        await browser.notifications.create({
          ...
          message: reason.startsWith("此频道")
            ? reason
            : browser.i18n.getMessage("errorCctvLiveUrlCaptureFailed"),
        });
        return { error: reason };
      }
    })();
